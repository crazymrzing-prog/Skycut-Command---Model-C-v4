...
from skycut_c_geometry import flatten_superpath_to_subpaths, dedupe, is_cw
from skycut_c_arrows import direction_arrows
from skycut_c_constants import CAPTURE_ENABLE_MASTER, CAPTURE_STROKE_WIDTH, FLATTEN_TOLERANCE_MM, PREVIEW_COLORS, LAYER_DISPLAY_NAMES
...
class SkyCutLayerMixin:
    ...
    def layer_exists(self, label):
        """True if a layer named `label` (case-insensitive) exists anywhere
        in the document, regardless of whether it contains any paths."""
    ...
    def preview_paths(self, sides):
        ...
        with 'side' (a name for this tool, e.g. 'left'/'right' or
        'tool'), 'kind' ('cut'/'score'/'draw'/'crease'/'none'), and
        ...
        Colors are the fixed PREVIEW_COLORS constants, not user-
        configurable.
        """
        ...
        for s in sides:
            ...
            if kind == "none":
                continue
            group = Group()
            group.label = f"{labels.get(kind, kind.title())} Preview ({s['side'].title()})"
            preview.add(group)

            path_style = style_for(kind)
            for idx, (pts, _) in enumerate(s["paths"]):
                if len(pts) > 1:
                    pe = PathElement()
                    pe.path = Path([["M", pts[0]]] + [["L", p] for p in pts[1:]])
                    pe.style = path_style
                    pe.label = f"{labels.get(kind, kind.title())}-{idx+1}"
                    group.add(pe)
    ...
    def convert_shapes_to_paths(self, elements):
        """
        Replace each element in `elements` (as returned by
        find_unconverted_shapes()) with the equivalent PathElement, in
        place at the same position in the document.

        Rect/circle/ellipse/polygon/polyline/line are simple
        parametric shapes - inkex can compute their outline
        analytically in pure Python via to_path_element(), so those go
        through _convert_shape_element() below.

        <svg:text> is NOT handled the same way: to_path_element() does
        not do real glyph outlining for text (confirmed by testing -
        it leaves the text unconverted rather than raising an error,
        which is worse than doing nothing since it fails silently).
        Turning glyphs into path geometry needs actual font-rendering
        data that plain inkex doesn't have access to, so text objects
        are instead routed through convert_text_to_paths(), which
        shells out to the real `inkscape` binary and uses its own
        Text > Object to Path conversion - the same one a person
        gets by selecting the text and running that menu command by
        hand.
